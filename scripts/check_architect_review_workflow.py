...
def check_workflow_text(text: str) -> list[CheckResult]:
    ...
    _add(results, "checkout uses actions/checkout@v5", _contains(review_job, "uses: actions/checkout@v5"))
    ...
    _add(results, "uses official Codex action v1", _contains(review_job, "uses: openai/codex-action@v1"))
    ...
    _add(results, "uses prompt file", _contains(review_job, "prompt-file: .github/codex/prompts/architect-review.md"))
    ...
    codex_index = review_job.find("uses: openai/codex-action@v1")
    ...
    _add(results, "comment includes workflow run ID", _contains(post_job, "context.runId"))
    ...
    _add(results, "deduplicates by reviewed SHA", _contains(post_job, "Reviewed commit:"))
