...
def check_collector(text: str) -> list[CheckResult]:
    ...
    add(results, "collector does not listen for synchronize", "- synchronize" not in text)
    add(results, "collector does not listen for reopened", "- reopened" not in text)
    add(results, "collector does not listen for ready_for_review", "- ready_for_review" not in text)
    add(results, "collector requires proposal label", "github.event.label.name == 'ai-fix-proposal'" in collect_job)
    ...
    add(results, "collector writes fixed request stage", "FIX_PROPOSAL_REQUEST" in collect_job)
    add(results, "collector records event label", "event_label" in collect_job)
