...
def check_recorder(text: str) -> list[CheckResult]:
    ...
    add(results, "approval validates repository permission", "approval_actor_repository_permission" in script and "github_approval_actor_repository_permission" in script)
    add(results, "approval uses repository permission API", "/repos/{repo}/collaborators/{actor}/permission" in script)
    add(results, "approval does not require public org membership", "/orgs/" not in script and "github_approval_actor_membership" not in script)
    add(results, "approval rejects write permission", '"write"' in script or "write" in script)
...
def check_schema_and_scripts() -> list[CheckResult]:
    ...
    add(results, "approval schema records repository permission", '"approved_by_repository_permission"' in schema and '"admin"' in schema and '"maintain"' in schema)
    add(results, "approval schema records repository role", '"approved_by_repository_role"' in schema)
    add(results, "approval trusted script allows admin and maintain", 'ALLOWED_REPOSITORY_PERMISSIONS = {"admin", "maintain"}' in script)
    add(results, "approval trusted script does not allow write or lower", '"write"' not in script.split("ALLOWED_REPOSITORY_PERMISSIONS", 1)[1].split("SHA_RE", 1)[0])
