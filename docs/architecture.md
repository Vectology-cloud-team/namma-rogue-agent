...
The engine must not depend on HTTP, Python-specific runtime behavior, LLM providers, NaMMA providers, or curses. It may expose a narrow API that the environment layer can call.
...
The environment is the only layer that should expose game state to the agent.
...
- Episode Memory: keeps observations, plans, actions, and outcomes.
- Retry Manager: starts the next episode after terminal failure.

The planner should not emit raw Rogue keypresses. It should emit structured plans or semantic actions.
